...
use crate::{CHUNK_SIZE, MAX_HZ, OBS_KEY, SAMPLE_RATE};
...
pub fn modify_sound_spectrum(
    freq_samples: &mut Vec<Complex>,
    target_hz: f64,
    sample_rate: f64,
    magnitude: f32,
    phase: f32,
    ...
    if target_bin >= n {
        return Err(StegoError::FrequencyOutOfBounds {
            hz_rate: target_hz as f32,
        });
    }
...
pub fn dft_scaling_factor() -> f32 {
    ...
    let cal_peak = cal_idft
        .iter()
        .map(|s| s.abs())
        .fold(0.0f32, |m, x| m.max(x));
    let idft_scale_factor = if cal_peak > 0.001 {
        cal_peak / 0.5
    } else {
        1.0
    };
...
pub fn calculate_adaptive_magnitude(
    chunk: &[f32],
    chunk_size: usize,
    target_time_amplitude: f32,
) -> f32 {
    let peak_amplitude = chunk
        .iter()
