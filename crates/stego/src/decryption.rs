...
use crate::{CHUNK_SIZE, OBS_KEY, SAMPLE_RATE};

fn decode_secret_from_windows(
    decoded_chars: &mut Vec<u8>,
    freq_samples: &Vec<Complex>,
    base_k: usize,
    window_idx: usize,
) {
    let mut max_mag = 0.0;
    ...
    for k in base_k..(base_k + 256) {
        if k >= freq_samples.len() {
            break;
        }
        let mag = f32::sqrt(
            (freq_samples[k].re * freq_samples[k].re) + (freq_samples[k].im * freq_samples[k].im),
        );
        if mag > max_mag {
            max_mag = mag;
            best_k = k;
...
fn exctract_header_len(freq_samples: &Vec<Complex>, base_k: usize, msg_len: &mut usize) {
    ...
    for k in base_k..1024 {
        // Protect Nyquist limit
        if k >= freq_samples.len() {
            break;
        }
        let mag = f32::sqrt(
...
fn unmask_message(file_samples: Vec<f32>) -> Result<(), StegoError> {
    ...
    for chunk in normalized_samples.chunks(CHUNK_SIZE) {
        ...
        if window_idx > 0 && window_idx <= msg_len {
            decode_secret_from_windows(&mut decoded_chars, &freq_samples, base_k, window_idx);
        }

        window_idx += 1;

        if window_idx > msg_len && msg_len > 0 {
            break;
        }
    }

    if let Ok(secret_string) = String::from_utf8(decoded_chars) {
        println!("{}", secret_string);
    } else {
        return Err(StegoError::FailedToDecodeMessage {
            details: "Decoded bytes are not valid UTF-8".to_string(),
        });
    }
