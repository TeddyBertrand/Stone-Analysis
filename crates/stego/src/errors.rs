...
impl fmt::Display for StegoError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            StegoError::FrequencyOutOfBounds { hz_rate } => write!(
                f,
                "Warning: Frequency {} Hz is out of bounds for window size",
                hz_rate
            ),
            StegoError::FailedToWriteEncryptedAudio { details } => {
                write!(f, "Failed to write encrypted audio: {}", details)
            }
            StegoError::FailedToReadAudio { details } => {
                write!(f, "Failed to read audio: {}", details)
            }
            StegoError::FailedToDecodeMessage { details } => {
                write!(f, "Failed to decode message: {}", details)
            }
        }
    }
}
...
impl From<std::io::Error> for StegoError {
    fn from(err: std::io::Error) -> Self {
        StegoError::FailedToWriteEncryptedAudio {
