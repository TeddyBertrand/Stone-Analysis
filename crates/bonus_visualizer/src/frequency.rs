...
pub fn render(path: &str, output_path: &str) -> Result<(), AudioError> {
    ...
    let grid: Vec<Vec<f32>> = samples
        ...
        .map(|chunk| {
            let windowed: Vec<f32> = chunk.iter().enumerate().map(|(i, &s)| {
                let hann = 0.5 * (1.0 - (2.0 * std::f32::consts::PI * i as f32
                                         / (CHUNK_SIZE - 1) as f32).cos());
                s * hann
            }).collect();
    ...
    if grid.is_empty() { return Ok(()); }

    let max_mag = grid.iter().flat_map(|c| c.iter()).cloned()
        .fold(f32::NEG_INFINITY, f32::max);
    if max_mag == 0.0 { return Ok(()); }
