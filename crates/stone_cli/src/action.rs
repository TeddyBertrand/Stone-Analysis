...
impl Action {
    ...
    pub fn from_parsed(args: ParsedArgs) -> Result<Self, CliError> {
        ...
        let mut next_arg = || {
            pos.next().cloned().ok_or(CliError::MissingRequiredOption(
        ...
        match args.mode {
            Mode::Analyze => {
                ...
                let n = next_arg()?.parse::<usize>().map_err(|_| {
                    CliError::BadArgument("L'argument doit être un entier positif valide".into())
                })?;
            ...
            Mode::Decypher => Ok(Action::Decypher { input: next_arg()? }),
